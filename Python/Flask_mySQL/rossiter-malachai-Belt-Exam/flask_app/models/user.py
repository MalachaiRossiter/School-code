...
class User:
    ...
    def validate_user(cls, user):
        ...
        if len(user['first_name']) < 2:
            is_valid = False
            flash('First name needs to be longer, but im not going to say how much longer cause you can use your big brain', 'registration')
        if len(user['last_name']) < 2:
            is_valid = False
            flash('Last Name needs to be longer, but im not going to say how much longer cause you can use your big brain', 'registration')
    ...
    def authenticate_user(cls, user):
        ...
        if not existing_user:
            flash('this email or password is very very wrong', 'login')
